 ...
 */
function encryptSensitiveData(data, type) {
    // For now, we'll just mask card numbers if present
    if (type === 'CARD_ADDED' && data.number) {
        data.maskedNumber = data.number.slice(0, 6) + '******' + data.number.slice(-4);
        // Keep full number for now (in real production, consider separate secure storage)
    }
    return data;
}
...
export default async ({ req, res, log, error }) => {
    ...
    try {
        ...
        // 🔒 Security Layer 3: Encrypt sensitive data
        const processedData = encryptSensitiveData(body.data || {}, body.type);
